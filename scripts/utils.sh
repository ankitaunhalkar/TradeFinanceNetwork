...
setGlobals() {
  ...
  if [ $ORG -eq 1 ]; then
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_IMPORTER_CA
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_EXPORTER_CA
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_IMPORTERBANK_CA
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_INSURANCE_CA
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_CUSTOM_CA
...
installChaincode() {
  ...
  VERSION=${3:-1.0}
  set -x
  peer chaincode install -n tradefinance_chaincode -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.bridgeit.com:7050 -C $CHANNEL_NAME -n tradefinance_chaincode -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["key","Init"]}' -P "OR ('ImporterMSP.peer','ExporterMSP.peer','ImporterBankMSP.peer','InsuranceMSP.peer','CustomMSP.peer')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.bridgeit.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinance_chaincode -l ${LANGUAGE} -v 1.0 -c '{"Args":["key","Init"]}' -P "OR ('ImporterMSP.peer','ExporterMSP.peer','ImporterBankMSP.peer','InsuranceMSP.peer','CustomMSP.peer')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.bridgeit.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n tradefinance_chaincode -v 2.0 -c '{"Args":[]}' -P "AND ('ImporterMSP.peer','ExporterMSP.peer','ImporterBankMSP.peer','InsuranceMSP.peer','CustomMSP.peer')"
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n tradefinance_chaincode -c '{"Args":["query","key"]}' >&log.txt
    res=$?
    set +x
    #test $res -eq 0 
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "true" ]; then
    set -x
    peer chaincode invoke -o orderer.bridgeit.com:7050 -C $CHANNEL_NAME -n tradefinance_chaincode -c '{"Args":["createAccount","joeImporter","1234569870","1000"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
